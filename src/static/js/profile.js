document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (metaTitle)
      metaTitle.setAttribute('content', `${username}'s Profile - OI Checklist`);
    const metaDesc = document.querySelector('meta[property="og:description"]');
    if (metaDesc)
      metaDesc.setAttribute(
        'content',
        `View ${username}'s progress and statistics on OI Checklist.`,
      );
  ...
  function populateProfile(data, username2) {
    document.getElementById('profile-username').textContent = username2;
    ...
    if (data.lastActivityAt) {
      ...
      document.getElementById('profile-last-activity').textContent =
    ...
    document.getElementById('checklist-link').href = `/checklist/${username2}`;

...
function updateStatus(status, cell, name, source, year) {
  ...
  const scoreColor = getScoreColor(parseFloat(cell.dataset.score) || 0, status);
  cell.style.backgroundColor = scoreColor || '';
  const popup = document.getElementById('status-popup');
  const popupStatus = document.getElementById('popup-status');
...
function getScoreColor(score, status) {
  if (status === 2) return '#3ccd43'; // Solved = always full green
  if (status === 3) return '#e21f1f';
  if (status === 0) return null; // Unattempted/Failed — no score color
  if (score === 100) return '#64cd69';
  if (score >= 81)   return '#d5e21f';
  if (score >= 61)   return '#e2c51f';
  if (score >= 41)   return '#e2ab1f';
  if (score >= 21)   return '#e2631f';
  return '#e21f1f';
}
